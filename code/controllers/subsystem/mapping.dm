...
/datum/controller/subsystem/mapping/proc/setup_ruins()
	...
	if (badlands_ruins.len)
		seedRuins(badlands_ruins, CONFIG_GET(number/lavaland_budget), list(/area/badlands/unexplored), themed_ruins[ZTRAIT_BADLANDS_RUINS], clear_below = TRUE, mineral_budget = 21)
	// honk end
...
/datum/controller/subsystem/mapping/proc/setup_rivers()
	...
	for (var/ice_z in ice_ruins)
		var/river_type = HAS_TRAIT(SSstation, STATION_TRAIT_FORESTED) ? /turf/open/lava/plasma/ice_moon : /turf/open/misc/asteroid/snow/icemoon // honk openspace/icemoon > snow/icemoon (prevents chasms)
