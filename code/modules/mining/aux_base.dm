...
/obj/machinery/computer/auxiliary_base/proc/set_landing_zone(turf/T, mob/user, no_restrictions)
	...
	if(!no_restrictions)
		...
		for(var/i in 1 to colony_turfs.len)
			...
			if(!place)
				return BAD_COORDS
			if(!istype(place.loc, /area/lavaland/surface) && !istype(place.loc, /area/icemoon/surface/outdoors/unexplored) && !istype(place.loc, /area/icemoon/underground/unexplored)) // honk - aux base on icemoon
				return BAD_AREA
